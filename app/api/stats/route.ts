...
// Cache de 5 minutes — clé inclut userId pour isolation multi-tenant
...
export async function GET(req: NextRequest) {
  // Auth guard — isoler les stats par utilisateur
  ...
  try {
    const result = await getCachedStats(user.id)
    return NextResponse.json(result)
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : 'Erreur inconnue'
    console.error('[/api/stats GET]', message)
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
